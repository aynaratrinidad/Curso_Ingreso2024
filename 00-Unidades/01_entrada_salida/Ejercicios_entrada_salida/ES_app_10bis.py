...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        importe_ingresado = self.txt_importe.get()
        importe_descuento = self.txt_descuento.get()

        importe_ingresado = float(importe_ingresado)
        importe_descuento = float(importe_descuento)
        importe_actualizado = importe_ingresado * (1 - importe_descuento / 100)
        mensaje = f"Importe actualizado con descuento {importe_actualizado:.2F}"
