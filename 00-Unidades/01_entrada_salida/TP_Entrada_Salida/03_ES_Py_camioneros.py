...
3.	Para el departamento de logística:
	...
	A.	Es necesario saber la cantidad camiones que harian falta
    para transportar los materiales que se utilizarán para la construcción de un edificio. 
    Para ello, se ingresa la cantidad de toneladas necesarias de materiales a transportar.
    El programa deberá informar la cantidad de camiones, sabiendo que cada uno de ellos
    puede transportar por viaje 3500kg
...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ingresar_toneladas = float(self.txt_toneladas.get())
        camiones_necesarios = ingresar_toneladas / 3.5  
        mensaje = f"Se necesitan {int(camiones_necesarios)} camiones para transportar {ingresar_toneladas} toneladas."
        alert("Cantidad de Camiones", mensaje)

    def btn_tiempo_llegada_on_click(self):
        ingresar_kilometros = float(self.txt_kilometros.get())
        tiempo_llegada = ingresar_kilometros / 90 
        mensaje = f"El tiempo de llegada será de {tiempo_llegada:.2f} horas."
